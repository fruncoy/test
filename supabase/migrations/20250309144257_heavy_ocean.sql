...
-- Basic RLS policies with checks to prevent errors on existing policies
DO $$
BEGIN
  -- Profiles policies
  IF NOT EXISTS (
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'liked_shows' AND policyname = 'Users can view their liked shows'
  ) THEN
    CREATE POLICY "Users can view their liked shows" ON liked_shows
      FOR SELECT USING (auth.uid() = user_id);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'liked_shows' AND policyname = 'Users can manage their liked shows'
  ) THEN
    CREATE POLICY "Users can manage their liked shows" ON liked_shows
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'notification_settings' AND policyname = 'Users can view their notification settings'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'notification_settings' AND policyname = 'Users can manage their notification settings'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'chat_messages' AND policyname = 'Chat messages are viewable by everyone'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'chat_messages' AND policyname = 'Users can insert chat messages'
  ) THEN
...
-- Create handle new user function if it doesn't exist
